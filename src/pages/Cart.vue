...
<template>
  ...
  <form v-if="store.getters.isNotEmptyCart" class="cart" action="">
    <div class="cart__header header-cart">
      ...
      <button @click="store.commit('clearCart')" class="header-cart__clear-cart" type="button">Очистить корзину</button>
    </div>

    <div class="cart__products products-cart">
      <ul class="products-cart__list">
        <li v-for="item in store.state.cartProducts" :key="item.id" class="products-cart__item itemCart">
          <div class="itemCart__wrap">

            <figure class="itemCart__thumbnail thumbnail-itemCart"><img class="thumbnail-itemCart__image"
                :src="item.image.url" :alt="item.image.alt" width="100">
            </figure>
            <div class="itemCart__info info-itemCart">
              <p class="info-itemCart__title">{{ item.title }}</p>
              <p class="info-itemCart__description" v-html="item.description"></p>
              <p class="info-itemCart__article">Артикул: {{ item.article }}</p>
            </div>

            <div class="itemCart__quantity quantity-itemCart">
              <div class="quantity-itemCart__wrap" :class="{ 'quantity-itemCart__wrap--extend': item.quantity > 1 }">
                <button @click="store.commit('decrementProductsPiece', item)" class="quantity-itemCart__decrement"
                  :disabled="item.quantity === 0" type="button">
                  <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10">
                    <path stroke="currentColor" stroke-linecap="round" stroke-width="1" d="M 1,5 L9,5" />
                  </svg>
                </button>
                <p class="quantity-itemCart__select">{{ item.quantity }}</p>
                <button @click="store.commit('incrementProductsPiece', item)" class="quantity-itemCart__increment"
                  :disabled="item.quantity === item.inStock" :title="item.quantity === item.inStock ? 'Больше нету': ''" type="button">
                  <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10">
                    <path stroke="currentColor" stroke-linecap="round" stroke-width="1" d="M 1,5 L9,5 M 5,1 L5,9" />
                  </svg>
                </button>
              </div>
              <p class="quantity-itemCart__unit-price" v-if="item.quantity > 1">{{ item.price.toLocaleString() }} ₽/шт.
              </p>
            </div>
            <div class="itemCart__total total-itemCart">
              <p class="total-itemCart__price">{{ (item.price * item.quantity).toLocaleString() }} ₽</p>
            </div>
          </div>
          <div class="itemCart__remove remove-itemCart">
            <button @click="removeFromCart(item)" class="remove-itemCart__button" type="button"
              aria-label="Удалить из корзины" title="Удалить из корзины"><svg xmlns="http://www.w3.org/2000/svg"
                width="14" height="14">
                <path stroke="currentColor" stroke-width="1.5" d="M 1,1 L13,13 M 13,1 L1,13" />
              </svg></button>
          </div>
        </li>
      </ul>
      ...
      <div class="products-cart__installation-service installationService">
        <div class="installationService__approval">
          <input @change="store.commit('toggleInstallation', item)" class="installationService__approval-input sr-only"
            type="checkbox" name="installation" id="installation">
          <label for="installation" class="installationService__approval-iconChecked"></label>
          <label for="installation" class="installationService__approval-iconUnchecked"></label>

        </div>
        <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor"
          class="installationService__icon" aria-hidden="true">
          <path
        ...
        <dl class="installationService__text-wrap">
          ...
          <dd class="installationService__description">Отметьте, если Вам необходима консультация профессионала по
            монтажу выбранных товаров.</dd>
        </dl>
      </div>
    </div>
    ...
    <div class="cart__summary summary-cart">
      <div class="summary-cart__wrap">
        ...
        <dl class="summary-cart__info-list">
          ...
          <dd class="summary-cart__info-value">{{ store.getters.sumTotal.toLocaleString() }} ₽</dd>
        </dl>

        <dl class="summary-cart__info-list">
          <dt class="summary-cart__info-label">Количество</dt>
          <dd class="summary-cart__info-value">{{ store.getters.totalProductsPiece }} шт</dd>
        </dl>

        <dl class="summary-cart__info-list">
          <dt class="summary-cart__info-label">Установка</dt>
          <dd class="summary-cart__info-value">{{ store.state.installation ? "Да" : "Нет" }}</dd>
        </dl>

        <dl class="summary-cart__info-list summary-cart__info-list--summary">
          <dt class="summary-cart__info-label summary-cart__info-label">Стоимость товаров</dt>
          <dd class="summary-cart__info-value summary-cart__info-value">{{ store.getters.sumTotal.toLocaleString() }} ₽
          </dd>
        </dl>
        <div class="summary-cart__action-list">
          <button @click="sendOrder()" class="summary-cart__checkout" :disabled="store.getters.totalProductsPiece <= 0"
            type="button">Оформить заказ</button>
          <button class="summary-cart__buy-in-one-click" :disabled="store.getters.totalProductsPiece <= 0"
            type="button">Купить в 1 клик</button>
        </div>
      </div>
    </div>
  </form>
...
.installationService__approval-input:checked~.installationService__approval-iconChecked {
  display: block;
  padding: 4px;
...
.installationService__approval-input:not(:checked)~.installationService__approval-iconUnchecked {
  display: block;
}

.installationService__approval-input:focus-visible~.installationService__approval-iconChecked,
.installationService__approval-input:focus-visible~.installationService__approval-iconUnchecked {
  box-shadow:
    inset 2px 2px 1px rgba(0, 0, 0, 0.08),
    0 0 0 3px var(--c-medium-bg),
