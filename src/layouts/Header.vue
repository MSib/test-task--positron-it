  ...
  <header class="header">
    <div class="header__cart cart-header">
      <div class="cart-header__icon" aria-hidden="true">
        <svg xmlns="http://www.w3.org/2000/svg" width="29" height="26" fill="currentColor">
          <path
      ...
      <p v-if="store.getters.isNotEmptyCart" class="cart-header__count"><QuantityProductsWithText /></p>
      <p v-else class="cart-header__is-empty">Пусто</p>
      <p v-if="store.getters.isNotEmptyCart" class="cart-header__total-price">{{ store.getters.sumTotal.toLocaleString()
      }} ₽</p>
    </div>
  </header>
</template>
